...
// Middleware para extrair id_instituicao do usuário autenticado
router.use(authMiddleware);

// Listar todos os dias sem aula da instituição
...
// Verificar se uma data específica é dia sem aula
...
router.post('/', async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      ...
      [data, motivo || null, req.id_instituicao, req.userId]
    );
...
// Marcar todos os fins de semana de um ano
router.post('/marcar-fins-de-semana', async (req, res) => {
  try {
    ...
    // Inserir em lote
    const values = finsDeSemana.map(data => [data, 'Fim de semana', req.id_instituicao, req.userId]);
...
// Adicionar feriados nacionais do Brasil
router.post('/adicionar-feriados-nacionais', async (req, res) => {
  try {
    ...
    // Inserir em lote
    const values = feriados.map(f => [f.data, f.motivo, req.id_instituicao, req.userId]);
...
// Marcar período de dias sem aula
router.post('/marcar-periodo', async (req, res) => {
  try {
    ...
    // Inserir em lote
    const values = diasSemAula.map(data => [data, motivo || 'Período sem aula', req.id_instituicao, req.userId]);
